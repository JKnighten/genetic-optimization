...
public class ParaNQueensProblem implements IGenOptimizeProblem<NQueensIndividual> {
    ...
    /**
     * Calculates fitness for each individual in the population. For the ParaNQueensProblem fitness is calculated by
     * finding the number of conflicts that are on the board contained in the NQueensIndividuals. Queens are in conflict
    ...
    public void calculateFitness(List<NQueensIndividual> population) {
        population.parallelStream()
                .forEach(individual -> individual.setFitness(NQueensHelper.conflictScore(individual.getValue())));
    ...
    /**
     * Selects the selectionPercent percent of best NQueensIndividuals in the population. The best NQueensIndividuals are
     * the ones with the lowest fitness score, which with the least amount of conflicts.
     *
     ...
     * occurs by selecting two random NQueensIndividuals and then combining both of their board states. Board states are
     ...
     * beyond are are copied from the second individual into the new individual to complete its board.
     *
    ...
    private NQueensIndividual crossIndividuals(List<NQueensIndividual> selectedPopulation) {
        ...
        for(int column=0; column<n; column++)
            crossedBoard[column] = (column<crossPoint) ? individ1.getValue()[column] : individ2.getValue()[column];

        return new NQueensIndividual(crossedBoard);
    }

    /**
     * Randomly mutates the position of queens in the population of NQueensIndividuals
     *
    ...
    private void randomMutation(NQueensIndividual individual, double mutationProb){
        ...
        for(int column=0; column<n; column++)
            if(random.nextDouble() < mutationProb)
                individual.getValue()[column] = random.nextInt(n);
    }
    ...
    public static void main(String[] args) {
        ...
        double selectionPercent = 0.05;
        double mutationPercent = 0.01;


        // Setup Problem //
        IGenOptimizeProblem problem = new ParaNQueensProblem(128);
